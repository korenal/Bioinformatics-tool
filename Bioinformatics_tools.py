...
def fasta_option():
        ...
        for line in f:
            if len(line.strip()) != 0:
                ...
                if splitter[0][:1] == '>':
                    ...
                    string = '\n'.join(splitter)
                    if second_dict[sequences[num - 1]] is None:
    ...
    for sequence in sequences:
        ...
        num += 1
    Button(window, text="OK", width=30, command=lambda: fasta_output(
        num, first_dict, second_dict, sequences, r.get())).pack(anchor=SE)
...
def fasta_output(num, first_dict, second_dict, sequences, value):
    ...
    for i in range(0, num):
        if i == (value - 1):
            ...
            entry1.pack()
            entry2.pack()
            Button(window, text="OK", width=30, command=lambda: fasta_subsequence(
                              name, second_dict, entry1.get(), entry2.get())).pack(anchor=SE)
            Button(window, text="BACK TO MAINMENU", width=30, command=lambda: main_menu()).pack(anchor=SW)


# Measuring sequence similarity using Hamming distance
# Function which return Hamming distance for two sequences
...
# Function which return two sequences
def sequences(first, second, file):
    first -= 1
    second -= 1
    num = 0
...
def hamming_choice(value):
    ...
    if value == 1:
        ...
        Button(window, text="BACK", width=30, command=lambda: hamming_option()).pack(anchor=SW)
    # Input is manually entered sequence
    else:
        clearFrame()
        ...
        entry2.pack()
        Button(window, text="OK", width=30, command=lambda: hamming_worker(
            entry1.get(), entry2.get())).pack(anchor=SE)
        Button(window, text="BACK TO MAINMENU", width=30, command=lambda: main_menu()).pack(anchor=SW)


# Function which receive an input
def hamming_option():
    clearFrame()
...
# Function which return and variants of alignment
...
def edit_worker(first, second):
    ...
    edit_backtracking(matrix, first, second)
    Button(window, text="BACK TO MAINMENU", width=30, command=lambda: main_menu()).pack(anchor=SW)


# Function for receive an input
def edit_option():
...
# Function which return list of models from PDB
def get_model(file):
    ...
    for i in atoms:
        ...
        for j in atoms:
            ...
            if distance > max_distance:
                ...
                coordinates.append(atom_coordinates2)
    return max_distance, center, coordinates


# PDB parser
def click_pdb(value, file):
    clearFrame()
    ...
    # Write chains of the PDB file
        ...
        Button(window, text="BACK", width=30, command=lambda: pdb_option()).pack()
    # Write the residuums in PDB file
    elif value == 3:
        clearFrame()
    ...
    # Write summary information about PDB file
    elif value == 5:
        clearFrame()
        ...
        Button(window, text="BACK", command=lambda: pdb_option()).pack()
    # Write the width of the structure in PDB
    elif value == 6:
        clearFrame()
    ...
    # Write the HETATM atoms
        ...
        entry2 = Entry(window, width=30)
        entry2.pack()
        Button(window, text="OK", width=30, command=lambda: give_atoms(atoms, hetatm, entry2.get(),
                                                                       entry1.get())).pack(anchor=SE)
...
# Processing multiple sequence alignment
 ...
 # Function which process the scoring matrix
def get_scoring_matrix(scoring_file):
    scoring_matrix = []
...
# Function which return score for substitution in MSA
def get_scores(scoring_mat, adict):
    ANALPHABETS = [
        'A', 'R', 'N',
        'D', 'C', 'Q',
        'E', 'G', 'H',
...
# Function which work with the CLUSTAL file format
...
# Function which print the right sequence
def get_sequence_worker(value, sequence, adict, file):
    clearFrame()
...
# Function which recieve users input
def get_sequence(value, adict, file):
    clearFrame()
    entry = Entry(window, width=150)
    Label(window, text="Write sequence position or ID", font="none 12 bold").pack()
    entry.pack()
    Button(window, text="OK", width=30, command=lambda: get_sequence_worker(
        value, entry.get(), adict, file)).pack(anchor=SE)
    button = Button(window, text="BACK", width=30, command=lambda: multiple_seq_option()).pack(anchor=SW)


# Function which print the right column from MSA
def get_column_worker(num, adict, file):
    clearFrame()
    ...
    if int(num) < 0 and int(num) >= length:
        ...
        # adict2 - dictionary with MSA blocks, length - length of one column in MSA
        adict2, length = get_dictionary(file)
        column = 0
        num = int(num)
        num -= 1
        while num > length:
...
# Function which only receive an input
def get_column(adict, file):
    clearFrame()
    entry = Entry(window, width=150)
    Label(window, text="Write number of column of MSA", font="none 12 bold").pack()
    entry.pack()
    Button(window, text="OK", width=30, command=lambda: get_column_worker(
        entry.get(), adict, file)).pack(anchor=SE)
    Button(window, text="BACK", width=30, command=lambda: multiple_seq_option()).pack(anchor=SW)


# Function for evaluating sum of pairs of MSA
def get_sum_of_pairs(file):
    clearFrame()
    res = 0
    ...
    # adict - dictionary with MSA blocks, length - length of one column in MSA
...
# Function with 3 options
def click3(value, adict, file):
    clearFrame()
    if value == 1:
...
def multiple_seq_option():
    ...
    r3 = Radiobutton(window,
                     text="Retrieve sum of pairs score of a column and whole MSA with respect to given scoring matrix",
                     variable=r, value=3).pack()
...
def structure_click(val, file):
    ...
    if val == 1:
        ...
        num_buried = sum_of_amk(buried_stat)
        num_buried2 = sum_of_amk(buried_stat2)
        num_surface = sum_of_amk(surface_stat)
        num_surface2 = sum_of_amk(surface_stat2)
        Button(window, text="BACK", width=30, command=lambda: structure_option()).pack(anchor=SW)
...
def structure_option():
    ...
    r3 = Radiobutton(window, text="Output histogram of amino acids composition of buried and exposed amino acids",
                     variable=r, value=3).pack()
    r4 = Radiobutton(window, text="Quantify portion of polar amino acids in the core and on the surface od the protein",
                     variable=r, value=4).pack()
    r5 = Radiobutton(window, text="Use the second structure to quantify the portion surface and buried amino acids",
                     variable=r, value=5).pack()
